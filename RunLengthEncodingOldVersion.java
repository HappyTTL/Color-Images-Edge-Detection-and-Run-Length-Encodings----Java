...
public class RunLengthEncoding implements Iterable {
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
      if (runLengthList.length == 0) {
          return new RunIterator(new DListNode());
      } else {
          return new RunIterator(DList.head);
      }
  ...
  public PixImage toPixImage() {
    // Replace the following line with your solution.
      ...
      RunIterator currentIterator = this.iterator;
      if (currentIterator.next != null || currentIterator.prev != null){
          int W = 0;
          int H = 0;
          while(currentIterator.hasNext()){
              int[] temp = currentIterator.next();
              if (temp != null){
                  int[] currentArray = temp;
                  int L = currentArray[3];
                  while(L > 0){
                      decodedPixImage.setPixel(x,y,short(currentArray[0]),short(currentArray[1]),short(currentArray[2]));
                      L--;
                      W++;
                      if (W == width){
                          W = 0;
